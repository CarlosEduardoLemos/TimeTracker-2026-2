...
export function ReportsPage() {
  ...
  useEffect(() => {
    ...
    return () => { controller.abort(); activeExport.current?.abort(); };
  }, []);
  ...
  return <>
    ...
    <section className="card mt-5" aria-busy={!!exporting}><h2 className="font-bold">Filtros disponíveis</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><label className="text-xs font-semibold muted">Data<input type="date" className="form-field mt-1" value={date} max={todayIso()} onChange={event => { if (event.target.value) setDate(event.target.value); }} /></label><label className="text-xs font-semibold muted">Usuário<select className="form-field mt-1" value={username} onChange={event => setUsername(event.target.value)} disabled={!users}><option value="">Todos os usuários</option>{(users || []).map(user => <option key={user.username} value={user.username}>{user.full_name || user.username}</option>)}</select></label></div>
      ...
      {exporting && <p role="status" className="mt-3 text-sm muted">Preparando arquivo {exporting.toUpperCase()}…</p>}
