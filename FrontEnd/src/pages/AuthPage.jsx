...
export function AuthPage({ mode = 'login' }) {
  ...
  return <main className="grid min-h-screen place-items-center bg-page p-4 dark:bg-slate-950 dark:text-white">
    <section className="card w-full max-w-md">
      ...
      <div className="mt-5"><IntegrationNotice title="Autenticação indisponível">Cadastro e login dependem de endpoints, sessão e autorização no backend. Ainda não é possível criar conta nem entrar por esta interface.</IntegrationNotice></div>
      <a className="mt-5 inline-block text-sm font-semibold text-brand underline" href="#/painel">Ir para o painel</a>
    </section>
  </main>;
}
