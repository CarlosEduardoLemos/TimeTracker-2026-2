...
export function DashboardPage({ dark, toggleTheme }) {
  ...
  return <>
    ...
    <section className="mb-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4" aria-label="Filtros do resumo">
      <label className="text-xs font-semibold muted">Data do resumo<input className="form-field mt-1" type="date" value={date} max={todayIso()} onChange={event => { if (event.target.value) setDate(event.target.value); }} /></label>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5" aria-label="Indicadores disponíveis">
      <MetricCard label="Online" value={loading ? '…' : available.users && available.realtime ? count('online') : unavailable} detail="status de leitura da API; não confirma conexão" />
      ...
      <MetricCard label="Usuários cadastrados" value={loading ? '…' : available.users ? username ? data.users.filter(user => user.username === username).length : data.users.length : unavailable} detail="lista global retornada pela API" />
    </section>
    <section className="mt-5 grid gap-5 xl:grid-cols-2">
      <article className="card" aria-busy={loading}><h2 className="font-bold">Última atividade</h2><p className="mt-1 text-xs muted">Esta tabela é atual e não segue o filtro de data do resumo.</p><div className="mt-4 overflow-x-auto" tabIndex="0" aria-label="Tabela de última atividade com rolagem horizontal"><table className="w-full min-w-[580px] text-left text-sm"><caption className="sr-only">Última atividade dos usuários cadastrados</caption><thead><tr className="border-b"><th scope="col" className="py-2">Usuário</th><th scope="col">Estado</th><th scope="col">Aplicação</th><th scope="col">Última leitura</th></tr></thead><tbody>{filtered.map(person => <tr key={person.username} className="border-b border-slate-100 dark:border-slate-800"><th scope="row" className="py-3 font-semibold">{person.full_name || person.username}</th><td>{person.status === 'offline' ? 'Sem leitura recente' : person.status}</td><td>{person.realtime?.process_name || '—'}</td><td>{person.realtime ? `${person.realtime.seconds_since_last_activity}s atrás` : '—'}</td></tr>)}</tbody></table>{loading && <p role="status" className="py-6 text-center text-sm muted">Carregando atividade…</p>}{!loading && available.users && available.realtime && !filtered.length && <p className="py-6 text-center text-sm muted">Nenhum usuário encontrado.</p>}{!loading && (!available.users || !available.realtime) && <p className="py-6 text-center text-sm muted">Dados de atividade indisponíveis.</p>}</div></article>
