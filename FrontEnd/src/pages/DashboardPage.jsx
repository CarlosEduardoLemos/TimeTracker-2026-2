...
export function DashboardPage({ dark, toggleTheme }) {
  ...
  return (
    <>
      ...
      />
      <div className="mb-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-100">
      ...
      {error && (
        ...
        >
          {error}{' '}
          <button className="ml-2 font-bold underline" onClick={refresh}>
            Tentar novamente
          </button>
        </div>
      )}
      ...
      >
        <MetricCard
          label="Online"
          value={
            loading ? '…' : available.users && available.realtime ? count('online') : unavailable
          }
        ...
        <MetricCard
          label="Ausentes"
          value={
            loading ? '…' : available.users && available.realtime ? count('ausente') : unavailable
          }
        ...
        <MetricCard
          ...
          value={
            loading ? '…' : available.users && available.realtime ? count('offline') : unavailable
          }
        ...
        <MetricCard
          ...
          value={
            loading
              ? '…'
              : available.summary
                ? fmtDuration(totalSeconds(data.summary))
                : unavailable
          }
        ...
        <MetricCard
          ...
          value={
            loading
              ...
              : available.users
                ? username
                  ? data.users.filter((user) => user.username === username).length
                  : data.users.length
                : unavailable
          }
      ...
      <section className="mt-5 grid gap-5 xl:grid-cols-2">
        <article className="card min-w-0" aria-busy={loading}>
          ...
          >
              ...
              <tbody>
                {filtered.map((person) => (
                  ...
                  >
                    ...
                    <td>{person.status === 'offline' ? 'Sem leitura recente' : person.status}</td>
                    <td>{person.realtime?.process_name || '—'}</td>
                    <td>
                      {person.realtime
                        ? `${person.realtime.seconds_since_last_activity}s atrás`
                        : '—'}
                    </td>
                  </tr>
                ))}
            ...
            {loading && (
              <p role="status" className="py-6 text-center text-sm muted">
                Carregando atividade…
              </p>
            )}
            {!loading && available.users && available.realtime && !filtered.length && (
              <p className="py-6 text-center text-sm muted">Nenhum usuário encontrado.</p>
            )}
            {!loading && (!available.users || !available.realtime) && (
              <p className="py-6 text-center text-sm muted">Dados de atividade indisponíveis.</p>
            )}
          </div>
        </article>
        <article className="card">
          ...
          <p className="mt-1 text-xs muted">
            Categorias do resumo diário, sem classificação por task.
          </p>
            ...
            {available.summary ? (
              cats.length ? (
                cats.map((category) => (
                  <div
                    key={category.name}
                    className="flex items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-3 dark:bg-slate-800"
                  >
                    <span className="text-sm font-semibold">{category.name}</span>
                    <span className="text-sm muted">{fmtDuration(category.seconds)}</span>
                  </div>
                ))
              ) : (
                <p className="text-sm muted">Sem registros na data selecionada.</p>
              )
            ) : (
              <p className="text-sm muted">{loading ? 'Carregando…' : 'Resumo indisponível.'}</p>
            )}
          </div>
        </article>
      </section>
